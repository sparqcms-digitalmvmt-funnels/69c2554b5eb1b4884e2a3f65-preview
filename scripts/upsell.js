...
const UPSELL_NEXT_PAGE_SLUG = "product/1/thankyou";
    ...
    const response = await fetch(
      `https://app-cms-api-proxy-dev-001.azurewebsites.net/vrio/order_offers`,
      {
        ...
        body: JSON.stringify({
          ...
          order_id: lastOrderId,
          pageId: "8Ufr7AooKo1XaP8TBbHga0AYwt6cMvfaSnON2jFkrGKC87xtV-kNOXnoqTv-cDwL"
        })
      }
    );
...
const processUpsell = async () => {
  ...
  try {
    const orderData = JSON.parse(sessionStorage.getItem("orderData"));
    orderData.pageId = "8Ufr7AooKo1XaP8TBbHga0AYwt6cMvfaSnON2jFkrGKC87xtV-kNOXnoqTv-cDwL";
    const lastOrderId = sessionStorage.getItem("cms_oid");
